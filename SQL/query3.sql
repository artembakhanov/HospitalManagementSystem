WITH a1 as(SELECT patient_id, fname, lname, start_time FROM appointment NATURAL JOIN patient NATURAL JOIN general_user
), week1 as (
	SELECT patient_id, COUNT(patient_id) as num FROM a1 WHERE start_time BETWEEN CURRENT_DATE - INTERVAL '1 week' AND CURRENT_DATE
	GROUP BY patient_id
	HAVING count(patient_id) >= 2
), 	week2 as (
	SELECT patient_id, COUNT(patient_id) as num FROM a1
